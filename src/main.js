...
function journal() {
  ...
  <form id="log-form" class="form-grid"><label>Repas<select name="meal"><option>Petit-déjeuner</option><option>Déjeuner</option><option>Dîner</option><option>Collation</option></select></label><label>Aliment<select name="foodId">${state.foods.map((food) => `<option value="${food.id}">${food.name}</option>`).join('')}</select></label><label>Quantité (g)<input name="grams" type="number" min="1" value="100" required /></label><button>Ajouter</button></form></section>
...
function stock() {
  ...
  <section class="panel"><h2>Ajouter au stock</h2><form id="stock-form" class="form-grid"><label>Aliment<select name="foodId">${state.foods.map((food) => `<option value="${food.id}">${food.name}</option>`).join('')}</select></label><label>Quantité (g)<input name="quantity" type="number" min="0" value="100" required /></label><label>Alerte sous (g)<input name="minimum" type="number" min="0" value="50" required /></label><button>Ajouter</button></form></section>
...
function ingredientLine() {
  return `<div class="ingredient-line"><label>Ingrédient<select name="foodId">${state.foods.map((food) => `<option value="${food.id}">${food.name}</option>`).join('')}</select></label><label>Quantité (g)<input name="grams" type="number" min="1" value="100" required /></label></div>`;
}
function render() {
...
function bind() {
  ...
  app.querySelector('#log-form')?.addEventListener('submit', (event) => { event.preventDefault(); const data = new FormData(event.target); const food = state.foods.find((item) => item.id === data.get('foodId')); const grams = Number(data.get('grams')); const n = nutrientsFor(food, grams); state.logs.unshift({ id: crypto.randomUUID(), date: today(), meal: data.get('meal'), name: food.name, grams, foodId: food.id, ...n }); save(); notify('Aliment ajouté au journal.'); });
