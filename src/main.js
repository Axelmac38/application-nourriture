import { addNutrients, emptyState, foodName, lowStock, nutrientsFor, recipeNutrients, SAMPLE_FOODS } from './domain.js?v=20260919-24';
...
function shopping() {
  ...
  <section class="panel"><h2>Produits suivis chez Lidl</h2><p class="empty">${products.length} produits · classés par ordre alphabétique</p>${products.map((name) => { const records = history.filter((item) => item.name === name).sort((a, b) => a.date.localeCompare(b.date)); const latest = records.at(-1); return `<article class="tracked-product"><button class="product-toggle" data-price-product="${name}"><span><b>${name}</b><small>Lidl · ${latest.price.toFixed(2)} € · ${latest.date} · ${records.length} relevé${records.length > 1 ? 's' : ''}</small></span><strong>⌄</strong></button><div class="price-details" hidden data-price-details="${name}"><div class="price-store">Prix relevés chez Lidl · Saint-Martin-d’Hères</div>${priceChart(records)}<div class="price-history">${records.map((item) => `<span>${item.date} · ${item.price.toFixed(2)} €${item.quantity > 1 ? ` · ${item.quantity} unités` : ''}</span>`).join('')}</div><button class="small danger" data-remove-price-product="${name}">Supprimer ce produit</button></div></article>`; }).join('') || '<p class="empty">Aucun produit suivi.</p>'}</section>`;
}
function targets() {
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v=20260919-24');
render();
