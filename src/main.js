...
function stockEditor(item) {
  ...
  return `<dialog open class="target-dialog"><form id="stock-editor-form" data-stock-id="${item.id}" data-stock-unit="${unit}"><button class="close" type="button" data-close-stock-editor aria-label="Fermer">×</button><h2>Modifier le stock</h2><p>${foodName(state, item.foodId)}</p><label>Quantité disponible (${unit})<input name="quantity" type="number" min="0" step="${unit === 'unité' ? 1 : unit === 'kg' ? 0.1 : 1}" value="${shown.value}" required /></label><button class="save-targets">Enregistrer</button></form></dialog>`;
}
...
function bind() {
  ...
  app.querySelectorAll('[data-edit-stock]').forEach((article) => { let timer; const open = () => { const item = state.stock.find((stock) => stock.id === article.dataset.editStock); if (!item) return; app.insertAdjacentHTML('beforeend', stockEditor(item)); const dialog = app.querySelector('#stock-editor-form')?.closest('dialog'); dialog?.querySelector('[data-close-stock-editor]')?.addEventListener('click', () => dialog.remove()); dialog?.querySelector('#stock-editor-form')?.addEventListener('submit', (event) => { event.preventDefault(); const form = event.target; const data = new FormData(form); const unit = form.dataset.stockUnit; item.quantity = Math.max(0, quantityFromDisplay(item.foodId, Number(data.get('quantity')), unit)); if (unit !== 'unité') state.unitPreferences[item.foodId] = unit; save(); dialog.remove(); render(); }); }; article.addEventListener('dblclick', open); article.addEventListener('pointerdown', () => { timer = setTimeout(open, 600); }); article.addEventListener('pointerup', () => clearTimeout(timer)); article.addEventListener('pointerleave', () => clearTimeout(timer)); });
  ...
  app.querySelector('[data-copy-shopping]')?.addEventListener('click', async () => { try { await navigator.clipboard.writeText(shoppingExportText(selectedItems)); notify('Liste de courses copiée.'); } catch { notify('Copie impossible dans ce navigateur.'); } });
  app.querySelector('[data-export-shopping]')?.addEventListener('click', () => { const blob = new Blob([shoppingExportText(selectedItems)], { type: 'text/plain;charset=utf-8' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'liste-de-courses.txt'; link.click(); URL.revokeObjectURL(link.href); notify('Liste de courses extraite.'); });
  app.querySelector('[data-copy-stock]')?.addEventListener('click', async () => { try { await navigator.clipboard.writeText(stockExportText()); notify('Stock copié.'); } catch { notify('Copie impossible dans ce navigateur.'); } });
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v=20260920-83');
