...
function bind() {
  ...
  app.querySelectorAll('[data-shopping-quantity]').forEach((input) => { const output = document.createElement('output'); output.className = 'quantity-value'; output.dataset.shoppingQuantity = input.dataset.shoppingQuantity; output.textContent = input.value; input.replaceWith(output); });
  app.querySelectorAll('[data-shopping-step]').forEach((button) => button.addEventListener('click', () => { const name = button.dataset.shoppingName; const item = selectedShoppingItems(state.priceRecords || PRICE_RECORDS).find((entry) => entry.name === name); if (!item) return; const display = shoppingQuantityDisplay(item); const step = display.unit === 'kg' ? 0.1 : 1; const value = Math.max(1, number(display.value + Number(button.dataset.shoppingStep) * step)); state.shoppingQuantities = state.shoppingQuantities || {}; state.shoppingQuantities[name] = display.unit === 'kg' ? value * 1000 : value; save(); render(); }));
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v=20260920-97');
